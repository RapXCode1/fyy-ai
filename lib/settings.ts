// Prompt data is base64-encoded to prevent plain-text leaks in source control.
// Set FYY_* env vars in Vercel to override any value below.

const decode = (b: string): string => {
  try {
    if (typeof Buffer !== "undefined") return Buffer.from(b, "base64").toString("utf-8")
    return decodeURIComponent(
      Array.from(atob(b))
        .map((c) => "%" + c.charCodeAt(0).toString(16).padStart(2, "0"))
        .join("")
    )
  } catch { return "" }
}

const _SYS = ""
const _IDN = ""
const _BHV = ""
const _OWN = ""
const _GST = ""
const _LVE = ""

export const getSystemPrompt     = (): string => process.env.FYY_SYSTEM_PROMPT      || decode(_SYS)
export const getIdentityKnowledge = (): string => process.env.FYY_IDENTITY_KNOWLEDGE || decode(_IDN)
export const getBehaviorRules    = (): string => process.env.FYY_BEHAVIOR_RULES     || decode(_BHV)
export const getOwnerPrompt      = (): string => process.env.FYY_OWNER_PROMPT       || decode(_OWN)
export const getGuestInstruction = (): string => process.env.FYY_GUEST_INSTRUCTION  || decode(_GST)
export const getLiveInstruction  = (): string => process.env.FYY_LIVE_INSTRUCTION   || decode(_LVE)

export const FYY_SYSTEM_PROMPT      = getSystemPrompt()
export const FYY_IDENTITY_KNOWLEDGE = getIdentityKnowledge()

export let globalSettings = {
  get systemPrompt() { return getSystemPrompt() },
  temperature: 0.7,
  maxTokens: 4096,
  topP: 0.9,
  fontFamily: "Inter",
  themeStyle: "basic",
}

export function updateSettings(newSettings: Partial<Omit<typeof globalSettings, "systemPrompt">>) {
  globalSettings = { ...globalSettings, ...newSettings }
  return globalSettings
}
